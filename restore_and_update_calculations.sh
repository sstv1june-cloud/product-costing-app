...
LATEST_TAG=$(git tag -l "checkpoint-dev2-*" | sort -V | tail -n 1)
echo "==> 1. Found latest restore point: ${LATEST_TAG}"

echo "==> 2. Restoring pristine files from ${LATEST_TAG}..."
git checkout "${LATEST_TAG}" -- src/modules/module1-baseline/InlineEditModal.jsx src/shared/costCalculationService.js

echo "==> 3. Updating src/shared/costCalculationService.js with exact Excel formulas..."
...
echo "==> 4. Updating InlineEditModal.jsx with full 38 lines and manual tariff inputs..."
...
echo "==> 5. Restarting Vite dev server cleanly on port 5173..."
...
echo "-------------------------------------------------------------------"
echo "✅ Restored & updated calculations: All 38 lines visible and matching Excel!"
echo "-------------------------------------------------------------------"
