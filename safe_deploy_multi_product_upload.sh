...
BACKUP_DIR="backups/snapshot_before_multiupload_${TIMESTAMP}"

# 1. Create a full project restore point
echo "==> Creating full restore point in ${BACKUP_DIR}..."
mkdir -p "${BACKUP_DIR}"
cp -r src/ "${BACKUP_DIR}/" 2>/dev/null || true
cp package.json vite.config.js "${BACKUP_DIR}/" 2>/dev/null || true
git stash save "Restore Point Before Multi-Product Upload ${TIMESTAMP}" 2>/dev/null || true
git branch "restore-point-${TIMESTAMP}" 2>/dev/null || true
echo "✓ Restore point created: restore-point-${TIMESTAMP}"

# 2. Update BaselineMasterPage.jsx with vendor-safe multi-product parser (for BOTH Atomberg & Haier)
...
export default function BaselineMasterPage() {
  ...
  // Multi-Product Upload Modal States
  ...
  // Multi-Product Universal Parser for BOTH Atomberg & Haier
    ...
    reader.onload = (evt) => {
      try {
          ...
          for (let col = startCol; col < maxCols; col++) {
            ...
            rawRows.forEach(row => {
              ...
              if (label && val !== null && val !== '') {
                ...
                if (lLower.includes('part code') || lLower.includes('item code') || lLower.includes('part no') || lLower.includes('item no')) {
                  partCode = String(val).trim();
                } else if (lLower.includes('part name') || lLower.includes('component')) {
                  partName = String(val).trim();
                } else if (lLower.includes('rm grade') || lLower.includes('polymer') || lLower.includes('raw material')) {
                  rmGrade = String(val).trim();
                } else if (lLower.includes('rm base rate') || lLower.includes('rm rate')) {
                  rmBaseRate = Number(val) || (isHaier ? 140 : 140);
                } else if (lLower.includes('mb base') || lLower.includes('masterbatch rate')) {
                  mbBaseRate = Number(val) || 254;
                } else if (lLower.includes('mb %') || lLower.includes('masterbatch %') || lLower.includes('master batch required')) {
                  const n = Number(val) || 0;
                  mbPct = n < 1 ? n * 100 : n;
                } else if (lLower.includes('part weight') || lLower.includes('net wt') || lLower.includes('net weight')) {
                  partWt = Number(val) || (isHaier ? 197 : 37);
                } else if (lLower.includes('runner weight')) {
                  runnerWt = Number(val) || (isHaier ? 40 : 1);
                } else if (lLower.includes('cavity') || lLower.includes('no of cavity') || lLower.includes('no. of cavity')) {
                  cavity = Number(val) || 2;
                } else if (lLower.includes('cycle time')) {
                  cycleTime = Number(val) || (isHaier ? 48 : 47);
                } else if (lLower.includes('tonnage') || lLower.includes('m/c tonnage') || lLower.includes('machine tonnage')) {
                  tonnage = Number(val) || (isHaier ? 450 : 200);
                  shiftTariff = tonnage >= 650 ? 5760 : (tonnage <= 200 ? 2000 : 4600);
                } else if (lLower.includes('shift rate') || lLower.includes('tariff') || lLower.includes('hourly rate')) {
                  shiftTariff = Number(val) || shiftTariff;
        ...
        if (extracted.length === 0) {
          alert(`No new product columns found in Column D onwards for ${selectedVendor}. Please verify file structure.`);
          return;
        }
  ...
  // Vendor-Specific Multi-Product Template Generator (.xlsx)
...
echo "==> Safe deployment completed with zero impact to existing modules."
