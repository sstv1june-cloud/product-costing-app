...
echo "==> Updating calculation service with exact Net Weight-based Reconciliation formula..."
...
export function calculateHaierCost(params) {
  ...
  // 10. Shot Weight = (Net Weight * Cavity + Runner Weight)
  const shotWeightPerPiece = ((netWeight * cavity) + runnerWeight);
  
  // 11. Reconciliation Weight = Net Weight + 1.0% Melt Loss on Net Weight (= Net Weight * 1.01)
...
echo "==> Done! Reconciliation weight calculation aligned with Excel formula."
