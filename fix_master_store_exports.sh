...
export const addVendorBaselineProducts = (vendor, newProducts) => {
  ...
  const formatted = newProducts.map((p, idx) => ({
    id: p.id || `BL-${vendor.toUpperCase()}-${Date.now()}-${idx}`,
    vendor,
    itemCode: p.itemCode,
    componentName: p.componentName,
    model: p.model || 'Standard',
    mouldSize: p.mouldSize || '950*600*450',
    approvedRm: p.approvedRm || 'PP H110MA',
    approvedRmRate: Number(p.approvedRmRate || 135.83),
    masterbatchPct: Number(p.masterbatchPct || 4.0),
    masterbatchRate: Number(p.masterbatchRate || 258.54),
    bopCost: Number(p.bopCost || 0.0),
    cavity: Number(p.cavity || 2),
    netWeight: Number(p.netWeight || 37),
    runnerWeight: Number(p.runnerWeight || 1),
    cycleTimeApproved: Number(p.cycleTimeApproved || 47),
    cycleTime: Number(p.cycleTimeApproved || 47),
    machineTonnage: Number(p.machineTonnage || 200),
    hourlyRate: Number(p.hourlyRate || 250),
    validFrom: p.validFrom || "2026-08-01",
    parameters: {
      cavity: Number(p.cavity || 2),
      netWeightApproved: Number(p.netWeight || 37),
      runnerWeight: Number(p.runnerWeight || 1),
      cycleTimeApproved: Number(p.cycleTimeApproved || 47),
      machineTonnage: Number(p.machineTonnage || 200),
      shiftTariff: (Number(p.machineTonnage || 200) >= 600 ? 4800 : 2000),
      bopCost: Number(p.bopCost || 0.0),
      masterbatchPct: Number(p.masterbatchPct || 4.0)
    }
  }));
...
echo "==> MasterStore exports updated."
