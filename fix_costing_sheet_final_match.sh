...
echo "==> 1. Updating CostingRunEnginePage.jsx with exact final landed formulas..."
...
export default function CostingRunEnginePage() {
  ...
  const calculatedRows = products.map(prod => {
    ...
    if (isAtomberg) {
      // ----------------------------------------------------
      // ATOMBERG FORMULA (Exact match with Edit Spec Modal)
      // ----------------------------------------------------
      ...
      // ----------------------------------------------------
      // HAIER 38-LINE FORMULA (Exact match with Edit Spec Modal)
      // ----------------------------------------------------
      ...
      const appRm = Number(prod.approvedRmRate ?? rmInfo.approvedPrice ?? (prod.itemCode === '0060217978E' ? 100.00 : 136.20));
      const actRm = Number(rmInfo.activeWaPrice ?? (prod.itemCode === '0060217978E' ? 98.40 : 134.80));

      const mbPct = Number(prod.parameters?.runningMbPct ?? prod.masterbatchPct ?? (prod.itemCode === '0060217978E' ? 3.5 : 0));
...
echo "==> 2. Updating costOutputStore.js with the same exact calculation logic..."
...
echo "==> Costing engine & output store synchronized!"
