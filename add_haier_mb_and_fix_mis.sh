...
echo "==> 1. Updating MasterStore to include Haier MB in rmMatrix..."
...
echo "==> 2. Restarting server cleanly..."
rm -rf node_modules/.vite 2>/dev/null || true
killall -9 node 2>/dev/null || fuser -k 5173/tcp 2>/dev/null || true
nohup npm run dev -- --host 0.0.0.0 --port 5173 > /tmp/vite_server.log 2>&1 &
sleep 2

echo "==> Done! Haier MB is now live in RM Matrix & synchronized across all pages."
