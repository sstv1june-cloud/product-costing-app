...
echo "==> 1. Updating masterStore.js with robust case-insensitive vendor matching for audit logs..."
...
export const globalStore = {
  ...
  vendorLockStatus: saved?.vendorLockStatus || {
    Haier: true,
    Atomberg: true,
    LG: false,
    Whirlpool: false
...
export const updateVendorScheduleBulk = (vendor, validFrom, validTo, updatedRows) => {
  ...
  // Record into RM Price History Log with normalized vendor name
  updatedRows.forEach(uRow => {
    ...
    const note = priceChanged 
      ? `Approved price updated from ₹${(prev?.approvedPrice || uRow.approvedPrice).toFixed(2)} to ₹${Number(uRow.approvedPrice).toFixed(2)}`
      : `Schedule locked with ${altText}`;
...
export const updateBaselineParameters = ({ itemId, updatedItem, changeType, newValidFrom, reason } = {}) => {
  ...
  if (idx !== -1) {
    ...
    if (Math.abs(oldCycle - newCycle) >= 0.01) {
      changesList.push({ parameter: "Cycle Time", oldVal: `${oldCycle} s`, newVal: `${newCycle} s`, diff: `${newCycle - oldCycle > 0 ? '+' : ''}${newCycle - oldCycle} s` });
    }
    ...
    globalStore.parameterChangeLogs.unshift({
      ...
      costImpact: { oldCost: 10.0, newCost: 10.0, diff: 0.0 },
...
# 2. Update RMPriceMatrixPage.jsx auditLogs filter to be case-insensitive and robust
echo "==> 2. Ensuring RMPriceMatrixPage.jsx robustly matches vendor audit logs..."
# (The filter is already robust in masterStore, but let's check RMPriceMatrixPage filter)
...
echo "==> Done! Audit trail normalized for all vendors."
