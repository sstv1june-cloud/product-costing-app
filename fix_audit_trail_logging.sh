...
echo "==> 1. Updating masterStore.js to robustly capture and persist all audit logs..."
...
export const updateVendorScheduleBulk = (vendor, validFrom, validTo, updatedRows) => {
  ...
  // Record into RM Price History Log (Audit Trail)
...
export const updateBaselineParameters = ({ itemId, updatedItem, changeType, newValidFrom, reason } = {}) => {
  ...
  if (idx !== -1) {
    ...
    // Push to Parameter Change Log (Parameter Audit Log)
...
echo "==> Done! Audit trail and parameter change logs are now fully recorded and persistent."
