...
echo "==> 1. Verifying git history and restore tags..."
git log -1 --oneline
git tag -n1

echo "==> 2. Writing complete costCalculationService.js with all 4 named exports..."
cat << 'SERVICE_EOF' > src/shared/costCalculationService.js
import { getActiveRmMapping, getActiveMbMapping } from './masterStore';

// 1. Atomberg 38-Line Formula Calculation
...
// 2. Haier 18-Line Formula Calculation
...
export function calculatePieceCostUnified({ item, isBaseline = false, targetDate = null }) {
  ...
  if (isAtomberg) {
    ...
    const mbApprovedRate = isCrisper ? Number(mbMapping.approvedMbPrice || 240.00) : Number(item.masterbatchRate || 0.0);
