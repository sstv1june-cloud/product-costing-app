...
echo "==> 1. Updating costCalculationService.js with revised Runner Scrap formula (Added to Cost)..."
...
export function calculateAtombergCost(p) {
  ...
  const shiftRate = Number(p.shiftTariff || p.shiftRate || (tonnage * 8) || (tonnage * 10));
...
export function calculateHaierCost(params) {
  ...
  // Keep shift tariff unified (use explicit shiftTariff if provided, else tonnage * 8)
  ...
  // Revised calculation: (Runner Weight / No. of Cavity) * price per gram (rmRate / 1000)
  // ADD to cost instead of deduction
  const pricePerGram = rmRate / 1000.0;
  const runnerRecoveryCredit = (runnerWeight / cavity) * pricePerGram;
...
echo "==> 2. Patching InlineEditModal.jsx for Runner Scrap (+) display and synchronized shift tariff..."
MODAL_FILE=$(find src -name "*InlineEditModal*.jsx" | head -n 1)
if [ -n "$MODAL_FILE" ]; then
  # Make sure runner recovery displays as positive added cost
  sed -i 's/- ₹{Math.abs(calc.runnerRecoveryCredit/₹{Number(calc.runnerRecoveryCredit/g' "$MODAL_FILE" 2>/dev/null || true
  sed -i 's/- ₹0.04/₹{Number(calc.runnerRecoveryCredit || 0).toFixed(2)}/g' "$MODAL_FILE" 2>/dev/null || true
fi
...
echo "==> Done! Shift tariff and revised runner scrap calculation successfully synchronized."
