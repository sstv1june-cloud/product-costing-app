...
# 1. Update masterStore.js to store lock states and provide clean price getters
...
# 2. Update RMPriceMatrixPage.jsx to lock inputs and enable Unlock/Lock state toggles
cat << 'RM_PAGE_EOF' > src/modules/module2-rm-matrix/RMPriceMatrixPage.jsx
import React, { useState, useEffect } from 'react';
import { Sliders, Lock, Unlock, History, Calendar, CheckCircle2, ShieldCheck } from 'lucide-react';
...
# 3. Update InlineEditModal.jsx with exact Landed Cost summation (Base + ICC 1% + Freight)
...
export function calculateHaierCost(params) {
  ...
  const rmRate = Number(params.rmRate) || 140.00;
...
export function calculateAtombergCost(p) {
  // 1. Base + ICC + Freight summation
  const rmBase = Number(p.rmBase ?? 140.0);
  const rmIcc = rmBase * 0.01; // 1%
  const rmFreight = Number(p.rmFreight ?? 1.50);
  const rmLanded = rmBase + rmIcc + rmFreight; // Exact summation

  const mbBase = Number(p.mbBase ?? 254.0);
  const mbIcc = mbBase * 0.01; // 1%
  const mbFreight = Number(p.mbFreight ?? 2.00);
  const mbLanded = mbBase + mbIcc + mbFreight; // Exact summation
...
export default function InlineEditModal({ item, isOpen, onClose, onSave }) {
  ...
  // ---------- ATOMBERG VIEW ----------
  if (isAtomberg) {
    // Approved Column: pulls Approved Rate from RM Matrix (e.g. ₹140.00, ₹254.00)
    const approvedRmBase = Number(rmInfo.approvedPrice || 140.00);
    const approvedMbBase = Number(mbInfo.approvedMbPrice || 254.00);

    // Actual Column: pulls Selected Alternate WA Price from RM Matrix (e.g. ₹135.83, ₹258.54)
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden">
            <table className="min-w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-200 font-medium">
                ...
                {/* RM Base Rate: Approved vs Actual from RM Matrix */}
                ...
                {/* RM Landed Cost = Base + ICC + Freight */}
                ...
                {/* MB Base Rate: Approved vs Actual from RM Matrix */}
                ...
                {/* MB Landed Cost = Base + ICC + Freight */}
                ...
                <tr className="bg-purple-50/30">
                  ...
                  <td className="p-2 text-right font-mono font-bold bg-slate-50">{baseMbPctVal(baseP.mbPct)}%</td>
                ...
                <tr className="bg-slate-900 text-white font-bold"><td className="p-2.5 text-center">38</td><td className="p-2.5 font-black text-amber-300 uppercase tracking-wider">Final Landed cost</td><td className="p-2.5 text-center font-mono">₹/pc</td><td className="p-2.5 text-right font-mono font-black text-amber-300 text-sm">₹{baseCalc.finalLanded.toFixed(2)}</td><td className="p-2.5 font-mono font-black text-emerald-300 text-sm bg-slate-800 text-right">₹{runCalc.finalLanded.toFixed(2)}</td><td className="p-2.5 text-right font-mono font-black text-white text-sm">₹ {costVariance <= 0 ? `${costVariance.toFixed(2)}` : `+${costVariance.toFixed(2)}`}</td></tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          <table className="min-w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-200 font-medium">
              ...
              <tr className="bg-slate-900 text-white font-bold"><td className="p-2.5 text-center">17</td><td className="p-2.5 font-black text-amber-300 uppercase tracking-wider">TOTAL COMPONENT BASELINE COST</td><td className="p-2.5 text-center font-mono">₹</td><td className="p-2.5 text-right font-mono font-black text-amber-300 text-sm">₹{baselineCalc.totalCost.toFixed(2)}</td><td className="p-2.5 font-mono font-black text-emerald-300 text-sm bg-slate-800 text-right">₹{runningCalc.totalCost.toFixed(2)}</td><td className="p-2.5 text-right font-mono font-black text-white text-sm">₹ {costVariance <= 0 ? `${costVariance.toFixed(2)}` : `+${costVariance.toFixed(2)}`}</td></tr>
            </tbody>
          </table>
        </div>
...
function baseMbPctVal(val) {
  return (val * 100).toFixed(2);
}
MODAL_EOF

echo "==> Lock states, Alternate RM/MB dynamic fetch, and landed cost summation deployed."
