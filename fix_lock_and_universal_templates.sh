...
echo "==> 1. Ensuring masterStore.js starts with isLocked: true by default and provides rock-solid lock state..."
cat << 'STORE_EOF' > patch_store_lock.py
...
STORE_EOF
python3 patch_store_lock.py

echo "==> 2. Updating RMPriceMatrixPage.jsx with universal templates and strict lock guards..."
...
export default function RMPriceMatrixPage() {
  ...
  const isLocked = storeState.isLocked !== false; // Default strictly to TRUE
  ...
  const handleAddPurchase = (e) => {
    e.preventDefault();
    if (isLocked) {
      alert("Page is Locked! Click 'Page Locked (Click to Unlock & Edit)' above to unlock before adding.");
      return;
    }
  ...
  const handleAddSale = (e) => {
    e.preventDefault();
    if (isLocked) {
      alert("Page is Locked! Click 'Page Locked (Click to Unlock & Edit)' above to unlock before recording dispatch.");
      return;
    }
  ...
  // Universal Multi-Vendor Purchase Inward Template
  const downloadUniversalPurchaseTemplate = () => {
    const aoa = [
      ["Date", "Vendor", "Invoice #", "Grade", "Inward Qty (kg)", "Purchase Rate (₹/kg)"],
      ["2026-08-01", "Haier Appliances", "INV-HR-001", "ABS 300-B Red (Prime Inward)", 5000, 134.80],
      ["2026-08-05", "Haier Appliances", "INV-HR-002", "White MB Grade A", 300, 250.00],
      ["2026-08-10", "Atomberg Technologies", "INV-AT-001", "PP H110MA Prime Inward", 4500, 133.80],
      ["2026-08-12", "Atharva Polymer", "INV-ATH-001", "HIPS-SH03 Prime Lot", 4000, 157.46]
    ];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Purchase_Inward");
    XLSX.writeFile(wb, "Universal_Purchase_Inward_Template.xlsx");
  };

  // Universal Multi-Vendor Sales Dispatch Template
  const downloadUniversalSalesTemplate = () => {
    const aoa = [
      ["Date", "Vendor", "Item Code", "Component Name", "Dispatch Qty", "Selling Price (₹)"],
      ["2026-08-01", "Haier Appliances", "0060217989D", "End cap Bottom Ref-ABS-DC-195,220", 4200, 42.00],
      ["2026-08-05", "Haier Appliances", "0060217978E", "CRISPER GPPS LV + 3.5% SMOKE GREY VEG BOX", 1800, 85.00],
      ["2026-08-10", "Atomberg Technologies", "A1017031_tt2", "Aris Top Canopy- Gloss Black", 3000, 15.96],
      ["2026-08-12", "Atharva Polymer", "0060235291A", "FRZ DUCT-FRONT COVER-HIPS-TM-258/278", 2200, 81.98]
    ];
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Sales_Dispatch");
  ...
  const handlePurchaseFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        alert(`Successfully imported ${count} purchase records mapped across specified vendors!`);
      } catch (err) {
        console.error(err);
  ...
  const handleSalesFileUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        alert(`Successfully imported ${count} sales dispatch records mapped across specified vendors!`);
      } catch (err) {
        console.error(err);
    ...
    <div className="space-y-4 text-xs font-sans">
      ...
      {isLocked && (
        <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-2.5 rounded-xl text-xs flex items-center gap-2">
          ...
          <span><strong>Protected State:</strong> This page is locked. Click <strong>"Page Locked (Click to Unlock & Edit)"</strong> above to edit Approved Price, modify alternates, or upload purchases/sales.</span>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-300 shadow-sm flex flex-wrap justify-between items-center gap-3">
            <form onSubmit={handleAddPurchase} className="flex flex-wrap items-center gap-2 flex-1">
              ...
              <input type="date" disabled={isLocked} value={newPurchaseDate} onChange={e => setNewPurchaseDate(e.target.value)} className="px-2 py-1.5 border rounded-lg text-xs disabled:bg-slate-100" />
              <input type="text" disabled={isLocked} placeholder="Inward RM Grade" value={newPurchaseGrade} onChange={e => setNewPurchaseGrade(e.target.value)} className="px-2.5 py-1.5 border rounded-lg font-mono text-xs w-48 disabled:bg-slate-100" />
              <input type="number" disabled={isLocked} step="0.1" placeholder="Qty (kg)" value={newPurchaseQty} onChange={e => setNewPurchaseQty(e.target.value)} className="px-2 py-1.5 border rounded-lg font-mono text-xs w-24 text-right disabled:bg-slate-100" />
              <input type="number" disabled={isLocked} step="0.01" placeholder="Rate (₹/kg)" value={newPurchaseRate} onChange={e => setNewPurchaseRate(e.target.value)} className="px-2 py-1.5 border rounded-lg font-mono text-xs w-24 text-right disabled:bg-slate-100" />
              <input type="text" disabled={isLocked} placeholder="Invoice #" value={newPurchaseInvoice} onChange={e => setNewPurchaseInvoice(e.target.value)} className="px-2 py-1.5 border rounded-lg font-mono text-xs w-28 disabled:bg-slate-100" />
              <button type="submit" disabled={isLocked} className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center gap-1 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={downloadUniversalPurchaseTemplate}
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-300 shadow-sm flex flex-wrap justify-between items-center gap-3">
            <form onSubmit={handleAddSale} className="flex flex-wrap items-center gap-2 flex-1">
              ...
              <input type="date" disabled={isLocked} value={newSaleDate} onChange={e => setNewSaleDate(e.target.value)} className="px-2 py-1.5 border rounded-lg text-xs disabled:bg-slate-100" />
              <input type="text" disabled={isLocked} placeholder="Item Code" value={newSaleItemCode} onChange={e => setNewSaleItemCode(e.target.value)} className="px-2.5 py-1.5 border rounded-lg font-mono text-xs w-36 disabled:bg-slate-100" />
              <input type="text" disabled={isLocked} placeholder="Component Description" value={newSaleCompName} onChange={e => setNewSaleCompName(e.target.value)} className="px-2.5 py-1.5 border rounded-lg text-xs w-52 disabled:bg-slate-100" />
              <input type="number" disabled={isLocked} step="1" placeholder="Dispatch Qty" value={newSaleQty} onChange={e => setNewSaleQty(e.target.value)} className="px-2 py-1.5 border rounded-lg font-mono text-xs w-28 text-right disabled:bg-slate-100" />
              <input type="number" disabled={isLocked} step="0.01" placeholder="Selling Price (₹)" value={newSalePrice} onChange={e => setNewSalePrice(e.target.value)} className="px-2 py-1.5 border rounded-lg font-mono text-xs w-28 text-right disabled:bg-slate-100" />
              <button type="submit" disabled={isLocked} className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center gap-1 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={downloadUniversalSalesTemplate}
                className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 rounded-xl font-bold flex items-center gap-1.5 cursor-pointer text-xs"
              >
                <Download className="w-3.5 h-3.5 text-blue-600" /> Template (.xlsx)
              </button>
...
echo "==> 3. Verifying clean build with npm run build..."
npm run build
...
echo "-------------------------------------------------------------------"
echo "✅ SUCCESS! Default Locked State & Universal Multi-Vendor Templates live."
echo "-------------------------------------------------------------------"
