...
echo "==> Updating BaselineMasterPage.jsx with full vertical 38-line staging & inline edit modal..."
      ...
      try {
        ...
        for (let colIdx = 3; colIdx < headerRow.length; colIdx++) {
          ...
          for (let rowIdx = 1; rowIdx < data.length; rowIdx++) {
            ...
            } else if (desc.includes('insert') || desc.includes('bop cost')) {
              if (val !== undefined) bopCost = Number(val) || 0.14;
            }
          }
...
echo "==> Done! Full vertical staging & inline editing deployed."
