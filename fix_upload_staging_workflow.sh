...
echo "==> Updating BaselineMasterPage.jsx with Excel Staging & Confirmation modal..."
BASELINE_FILE="src/modules/module1-baseline/BaselineMasterPage.jsx"

cat << 'EOF_PAGE' > "$BASELINE_FILE"
import React, { useState, useEffect } from 'react';
import { 
  FileSpreadsheet, Download, Search, Edit3, Layers, History, Upload, CheckCircle2, X, AlertTriangle 
} from 'lucide-react';
...
export default function BaselineMasterPage() {
        ...
        for (let colIdx = 3; colIdx < headerRow.length; colIdx++) {
          ...
          for (let rowIdx = 1; rowIdx < data.length; rowIdx++) {
            ...
            if (desc.includes('item no') || desc.includes('part code')) {
              if (val) itemCode = val.toString();
            } else if (desc.includes('model')) {
              if (val) model = val.toString();
            } else if (desc.includes('raw material required') || desc.includes('rm grade')) {
              if (val) rmGrade = val.toString();
            } else if (desc.includes('master batch required') || desc.includes('mb %')) {
              if (val !== undefined) mbPct = Number(val) || 0;
            } else if (desc.includes('cavity')) {
              if (val !== undefined) cavity = Number(val) || 2;
            } else if (desc.includes('runner weight')) {
              if (val !== undefined) runnerWt = Number(val) || 40;
            } else if (desc.includes('net weight') || desc.includes('part weight')) {
              if (val !== undefined) netWt = Number(val) || 197;
            } else if (desc.includes('cycle time')) {
              if (val !== undefined) cycleTime = Number(val) || 48;
            } else if (desc.includes('machine used') || desc.includes('m/c tonnage')) {
              if (val !== undefined) tonnage = Number(val) || 450;
            }
          }

          parsedProducts.push({
            id: `prod-${Date.now()}-${colIdx}`,
  ...
  const confirmStagingImport = () => {
    ...
    import('../../shared/masterStore').then(({ addStagedProductsToBaseline }) => {
      ...
      setStagedData(null);
      alert(`Successfully staged and added ${stagedData.length} product(s) to ${selectedVendor} baseline!`);
      setTick(t => t + 1);
    });
  };
  ...
  return (
    <div className="space-y-4 text-xs font-sans">
      ...
      {showStagingModal && stagedData && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full p-6 space-y-4 border border-slate-300 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b pb-3">
              <div>
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600" /> Staging Preview: {selectedVendor} Product Import ({stagedData.length} Parts)
                </h2>
                <p className="text-xs text-slate-500">Please review the parsed parameters below before confirming final import into the baseline master.</p>
              </div>
              ...
              <table className="min-w-full text-xs text-left">
                <thead className="bg-slate-100 text-slate-700 font-bold uppercase text-[10px]">
                  <tr>
                    ...
                    <th className="p-2.5">RM Grade</th>
                    <th className="p-2.5 text-center">Cavity</th>
                    <th className="p-2.5">Net Wt</th>
                    <th className="p-2.5">Runner Wt</th>
                    <th className="p-2.5">Cycle Time</th>
                    <th className="p-2.5">Tonnage</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 font-medium">
                  {stagedData.map((p, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      <td className="p-2.5 font-mono font-bold text-blue-700">{p.itemCode}</td>
                      <td className="p-2.5 font-bold text-slate-900">{p.componentName}</td>
                      <td className="p-2.5 text-slate-700">{p.model}</td>
                      <td className="p-2.5 text-slate-800">{p.approvedRm}</td>
                      <td className="p-2.5 text-center font-mono font-bold">{p.cavity}</td>
                      <td className="p-2.5 font-mono">{p.netWeight}g</td>
                      <td className="p-2.5 font-mono">{p.runnerWeight}g</td>
                      <td className="p-2.5 font-mono text-amber-800 font-bold">{p.cycleTimeApproved}s</td>
                      <td className="p-2.5 font-mono">{p.machineTonnage}T</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t">
              ...
              <button onClick={confirmStagingImport} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl flex items-center gap-1.5 cursor-pointer shadow-sm">
                <CheckCircle2 className="w-4 h-4" /> Confirm & Add to Baseline ({stagedData.length})
              </button>
            </div>
          </div>
        </div>
...
echo "==> Done! Upload & Staging workflow deployed successfully."
