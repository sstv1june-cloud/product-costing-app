...
echo "==> 2. Updating costCalculationService.js to include all Lines 25-33 and exact production cost..."
...
echo "==> 3. Updating InlineEditModal.jsx to display all secondary operations and calculated cost..."
...
export default function InlineEditModal({ product, onClose, onSave, onDelete }) {
  ...
  const [formData, setFormData] = useState({
    ...
    partsPerShift: Number(product.partsPerShift) || 0,
  ...
  const baseCalc = isHaier 
    ? calculateHaierCost({
        ...
        partsPerShift: formData.partsPerShift,
  ...
  const runningCalc = isHaier 
    ? calculateHaierCost({
        ...
        partsPerShift: formData.runningCycleTime > 0 ? ((28800 / formData.runningCycleTime) * 0.95 * formData.runningCavity) : 0,
  ...
  const partsShiftApproved = formData.partsPerShift > 0 ? formData.partsPerShift : (((28800 / ctApproved) * 0.95) * Number(formData.cavity || 1)).toFixed(2);
...
git add -A
git commit -m "fix(haier): complete 38-line costing formula parity for secondary operations and efficiency" || echo "dev-v2 clean."
...
echo "-------------------------------------------------------------------"
echo "✅ SUCCESS! 38-line costing engine synced cell-to-cell with Excel."
echo "-------------------------------------------------------------------"
