...
# 1. Update the centralized calculation service
  ...
  if (isAtomberg) {
    ...
    if (isBaseline) {
      return calculateHaierCost({
        ...
        shiftTariff: Number(item.hourlyRate ? item.hourlyRate * 8 : (item.machineTonnage >= 650 ? 5760 : 4600)),
      ...
      return calculateHaierCost({
        ...
        shiftTariff: Number(params.runningShiftTariff ?? (params.runningTonnage >= 650 ? 5760 : 4600)),
...
# 2. Update MISIntelligencePage.jsx with live calculation matching Costing Engine
...
export default function MISIntelligencePage() {
  ...
  const analyzedRows = useMemo(() => {
    return salesData.map(sale => {
      ...
      // 2. Live Simulated Actual Calculation
...
echo "==> Central calculation service & MIS Intelligence fully synchronized."
