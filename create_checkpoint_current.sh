...
echo "==> 1. Staging all current working files..."
git add -A

echo "==> 2. Committing current snapshot to dev-v2..."
git commit -m "checkpoint: all 38 lines visible, manual machine tariff entry, Line 24 exact formula & store parity [${TIMESTAMP}]" || echo "Working tree already clean."
