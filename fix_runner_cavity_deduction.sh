...
echo "==> 1. Updating costCalculationService.js with (Runner Cost / Cavity) deduction formula..."
...
export function calculateHaierCost(params) {
  ...
  // Total RM Cost = (Raw Material Cost + Masterbatch Cost) - (Runner Cost / Cavity)
  const runnerRecoveryCredit = runnerCost / cavity;
  ...
  return {
    ...
    rawMaterialCost, masterbatchCost, runnerCost, runnerRecoveryCredit, totalRmCost,
...
echo "==> 2. Updating modal display for (Runner Cost / Cavity)..."
MODAL_FILE=$(find src -name "*InlineEditModal*.jsx" | head -n 1)
if [ -n "$MODAL_FILE" ]; then
  sed -i 's/runnerRecoveryCredit || calc.runnerCost/runnerRecoveryCredit/g' "$MODAL_FILE" 2>/dev/null || true
fi
...
echo "==> Done! Calculation updated successfully."
