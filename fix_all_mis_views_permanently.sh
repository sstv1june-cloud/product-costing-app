...
echo "==> 1. Exporting the single calculateDetailedCost helper..."
...
echo "==> 2. Generating the unified MIS component..."
cat << 'MIS_CODE_EOF' > /tmp/ExactMISComponent.jsx
...
echo "==> 3. Overwriting all MIS Page paths..."
MIS_FILES=$(grep -rnwl "Product Sales Realization & Costing Analysis" src/ || true)
for f in $MIS_FILES; do
  echo "Overwriting active file: $f"
  cp /tmp/ExactMISComponent.jsx "$f"
done
cp /tmp/ExactMISComponent.jsx src/modules/module4-mis/MISIntelligencePage.jsx 2>/dev/null || true

echo "==> 4. Clearing Vite Cache and Restarting Server..."
