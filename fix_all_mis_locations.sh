...
echo "==> 1. Finding all files containing MIS Intelligence..."
MIS_FILES=$(grep -rnwl "Product Sales Realization & Costing Analysis" src/ || grep -rnwl "Vendor & Product Sales P&L MIS Intelligence" src/ || true)

echo "Found active MIS file(s):"
echo "$MIS_FILES"

# 2. Update masterStore.js to ensure baselineList & salesData have consistent data
...
# 3. Create the self-contained MIS page template
cat << 'SELF_MIS_EOF' > /tmp/CompleteMISComponent.jsx
...
// Self-contained, bullet-proof Atomberg & Haier calculation logic
function calcDirectCost(item, isBaseline = false, targetDate = null) {
  ...
  if (isAtomberg) {
    const rmBase = isBaseline ? Number(rmMapping.approvedPrice || item.approvedRmRate || 140.0) : Number(rmMapping.activeWaPrice || 135.83);
    const mbBase = isBaseline ? Number(mbMapping.approvedMbPrice || item.masterbatchRate || 254.0) : Number(mbMapping.activeMbPrice || 258.54);
    ...
    const rmRate = isBaseline ? Number(rmMapping.approvedPrice || item.approvedRmRate || 140.0) : Number(rmMapping.activeWaPrice || 134.80);
    const netWeight = Number(isBaseline ? (item.netWeight || 197) : (params.runningNetWeight ?? item.netWeight ?? 197));
    const runnerWeight = Number(isBaseline ? (item.runnerWeight || 40) : (params.runningRunnerWeight ?? item.runnerWeight ?? 40));
    const cavity = Number(isBaseline ? (item.cavity || 2) : (params.runningCavity ?? item.cavity ?? 2));
    const mbPct = Number(item.masterbatchPct || 0) / 100;
    const mbRate = Number(item.masterbatchRate || (rmRate * 1.9));

    const shotWeightPerPiece = ((netWeight * cavity) + runnerWeight) / cavity;
    const reconciliationWeight = shotWeightPerPiece * 1.01;

    const rawMaterialCost = (reconciliationWeight / 1000) * rmRate * Math.max(0, 1 - mbPct);
    const masterbatchCost = (reconciliationWeight / 1000) * mbRate * mbPct;
    const runnerCredit = (runnerWeight / cavity / 1000) * (rmRate * 0.25);
    const totalRmCost = (rawMaterialCost + masterbatchCost) - runnerCredit;

    const cycleTime = Number(isBaseline ? (item.cycleTimeApproved || item.cycleTime || 48) : (params.runningCycleTime ?? 48));
    const tonnage = Number(isBaseline ? (item.machineTonnage || 450) : (params.runningTonnage ?? 450));
...
export default function MISIntelligencePage() {
  ...
  const analyzedRows = useMemo(() => {
    return salesData.map(sale => {
      ...
      const contractBaseline = calcDirectCost({ ...part, vendor: sale.vendor }, true, sale.invoiceDate);
      const actualUnitCost = calcDirectCost({ ...part, vendor: sale.vendor }, false, sale.invoiceDate);
...
# 4. Overwrite any active MIS component files found
for f in $MIS_FILES; do
  echo "Overwriting: $f"
  cp /tmp/CompleteMISComponent.jsx "$f"
done
cp /tmp/CompleteMISComponent.jsx src/modules/module4-mis/MISIntelligencePage.jsx 2>/dev/null || true

# 5. Clear Vite cache and restart server on 5173
...
echo "==> Verification complete! Server running on port 5173."
