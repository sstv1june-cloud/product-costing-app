...
echo "==> 1. Updating masterStore.js to strictly resolve RM & MB codes with 0 fallback and compound string support..."
cat << 'STORE_EOF' > patch_store_lookup.py
...
STORE_EOF
python3 patch_store_lookup.py

echo "==> 2. Updating InlineEditModal.jsx to parse composite RM + MB strings and fetch live matrix rates dynamically..."
...
echo "-------------------------------------------------------------------"
echo "✅ SUCCESS! Dynamic RM & MB matrix rate linkage is live!"
echo "-------------------------------------------------------------------"
